...
TestDiscoveryResult test_discover(String tests_dir)
{
	...
	if (fun_error_is_error(dir_exists.error)) {
		...
		if (fun_error_is_ok(ar.error)) result.modules = ar.value;
		return result;
	}
	if (!dir_exists.value) {
	...
	ErrorResult list_result = fun_filesystem_list_directory(tests_dir, list_mem.value);
	...
	while (*ptr != '\0') {
		...
		for (StringLength i = 0; i < entry_len; i++) { entry_name[i] = ptr[i]; }
		...
		for (int i = 0; i < 6; i++) { file_ptr[path_len + 1 + i] = test_c[i]; }
		file_ptr[path_len + 1 + 6] = '\0';
		module.test_file = file_mem.value;

		ErrorResult push_result = fun_array_TestModule_push(&result.modules, module);
...
int test_has_test_file(String dir_path)
{
	...
	if (len + 7 >= sizeof(test_c_path)) { return 0; }
