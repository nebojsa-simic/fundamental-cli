...
ErrorResult fun_hashmap_put(HashMap *map, const void *key, const void *value) {
  ...
  HashMapEntry *existing = hashmap_find_entry_in_chain(map->buckets[bucket_idx],
                                                       key, map->equals_fn);
  ...
  if (fun_error_is_error(key_result.error)) {
    ...
    return fun_error_result(ERROR_CODE_HASHMAP_FULL, "Failed to allocate key");
  }
...
ErrorResult fun_hashmap_remove(HashMap *map, const void *key) {
  ...
  return fun_error_result(ERROR_CODE_KEY_NOT_FOUND, "Key not found in hashmap");
}
