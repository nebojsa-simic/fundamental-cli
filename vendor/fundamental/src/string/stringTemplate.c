      ...
      if (*(t + 1) == '{') {
        ...
        if (*end == '}') {
          ...
          for (size_t i = 0; i < paramCount; i++) {
            size_t j = 0;
            while (params[i].key[j] && key[j] && params[i].key[j] == key[j])
              j++;
          ...
          if (found) {
            ...
            case '$': // String
              ...
              break;
            case '#': // Integer
              fun_string_from_int(found->value.intValue, 10, buffer);
              break;
            case '%': // Double
              fun_string_from_double(found->value.doubleValue, 3, buffer);
              break;
            case '*': // Pointer
              fun_string_from_pointer(found->value.pointerValue, buffer);
              break;
            }
