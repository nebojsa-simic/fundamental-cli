...
ErrorResult fun_rbtree_insert(RBTree *tree, const void *key,
                              const void *value) {
  if (!tree || !key) {
  ...
  while (x != tree->nil) {
    ...
    if (tree->equals_fn(new_node->key, x->key)) {
      ...
      return fun_error_result(ERROR_CODE_KEY_EXISTS, "Key already exists");
...
ErrorResult fun_rbtree_get(const RBTree *tree, const void *key,
  ...
  if (!node) {
    return fun_error_result(ERROR_CODE_KEY_NOT_FOUND, "Key not found in tree");
  }
