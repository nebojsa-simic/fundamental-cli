...
AsyncResult fun_stream_open(String file_path, StreamMode mode, Memory buffer,
  ...
  if (fun_error_is_error(stream_result.error)) {
    return (AsyncResult){.status = ASYNC_ERROR, .error = stream_result.error};
  }
  ...
  *stream = (FileStream){.file_path = file_path,
  ...
  if (fun_error_is_error(state_result.error)) {
    ...
    return (AsyncResult){.status = ASYNC_ERROR, .error = state_result.error};
  }
...
AsyncStatus poll_stream_open(AsyncResult *result) {
  ...
  if (!state->file_opened) {
    ...
    int chars_converted = MultiByteToWideChar(CP_UTF8, 0, stream->file_path, -1,
                                              wide_path, MAX_PATH);
    ...
    DWORD access =
        (stream->mode == STREAM_MODE_READ) ? GENERIC_READ : GENERIC_WRITE;
    DWORD creation =
        (stream->mode == STREAM_MODE_WRITE) ? CREATE_ALWAYS : OPEN_EXISTING;

    state->file_handle = CreateFileW(wide_path, access, FILE_SHARE_READ, NULL,
                                     creation, FILE_ATTRIBUTE_NORMAL, NULL);

    if (state->file_handle == INVALID_HANDLE_VALUE) {
      result->error = fun_error_result(GetLastError(), "Failed to open file");
      return ASYNC_ERROR;
    }
