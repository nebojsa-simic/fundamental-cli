...
static int sys_close(int fd) { return (int)syscall1(SYS_close, fd); }
...
AsyncStatus poll_stream_read(AsyncResult *result) {
  ...
  off_t seek_result =
      sys_lseek(stream_state->file_descriptor,
                (off_t)state->stream->current_position, SEEK_SET);
  if (seek_result < 0) {
...
AsyncResult fun_stream_read(FileStream *stream, uint64_t *bytes_read) {
  ...
  MemoryResult state_result = fun_memory_allocate(sizeof(StreamReadAsyncState));
  ...
  *state = (StreamReadAsyncState){
      .stream = stream, .bytes_read = bytes_read, .read_complete = false};
