...
AsyncStatus poll_mmap_write(AsyncResult *result) {
  ...
  if (state->file_handle == INVALID_HANDLE_VALUE) {
    ...
    state->file_handle = CreateFileW(
        wide_path_buffer, GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ, NULL,
  ...
  if (!state->mapping_handle) {
    state->mapping_handle =
        CreateFileMappingW(state->file_handle, NULL, PAGE_READWRITE, 0,
  ...
  if (!state->mapped_view) {
    ...
    uint64_t view_size = state->parameters.bytes_to_write +
    ...
    if (!FlushFileBuffers(state->file_handle)) {
      result->error =
          (ErrorResult){.code = GetLastError(),
                        .message = "Failed to flush file buffers to disk"};
      final_status = ASYNC_ERROR;
      goto cleanup;
    }
