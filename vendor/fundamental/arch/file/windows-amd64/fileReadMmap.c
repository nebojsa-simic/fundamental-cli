...
AsyncStatus poll_mmap(AsyncResult *result) {
  ...
  if (!state->file_handle) {
    ...
    state->file_handle =
        CreateFileW(wide_path_buffer, GENERIC_READ, FILE_SHARE_READ, NULL,
                    OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
    if (state->file_handle == INVALID_HANDLE_VALUE) {
      result->error =
          (ErrorResult){.code = GetLastError(),
                        .message = "CreateFileW error, INVALID_HANDLE_VALUE"};
      final_status = ASYNC_ERROR;
  ...
  if (!state->mapping_handle) {
    ...
    state->mapping_handle =
        CreateFileMappingW(state->file_handle, NULL, PAGE_READONLY, 0, 0, NULL);
  ...
  if (!state->mapped_view) {
    ...
    if (!state->mapped_view) {
      result->error =
          (ErrorResult){.code = GetLastError(),
                        .message = "MapViewOfFile error, mapped_view null"};
      // Set status and jump to cleanup
      final_status = ASYNC_ERROR;
      goto cleanup;
    }
...
cleanup:
  ...
  if (state) { // Check if state was allocated
    ...
    if (state->file_handle &&
        state->file_handle != INVALID_HANDLE_VALUE) { // Check handle validity
