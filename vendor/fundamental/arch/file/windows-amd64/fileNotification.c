...
DWORD WINAPI file_monitor_thread(LPVOID param) {
  ...
  while (!monitoring_should_stop) {
    ...
    if (result) {
      ...
      DWORD waitResult = WaitForSingleObjectEx((HANDLE)state->overlapped.hEvent,
      ...
      if (waitResult == WAIT_OBJECT_0 && state->callback) {
        ...
        if (len >= 256)
          len = 255;
        fun_memory_copy(notifyInfo->FileName, fileName, len * sizeof(WCHAR));
        fileName[len] = L'\0';

        // Convert back to UTF-8 if needed and call the callback
        char utf8_filename[512];
        WideCharToMultiByte(CP_UTF8, 0, fileName, -1, utf8_filename, 512, NULL,
                            NULL);
...
AsyncResult fun_register_file_change_notification(String filePath,
  ...
  MemoryResult stateResult = fun_memory_allocate(sizeof(FileNotificationState));
  if (fun_error_is_error(stateResult.error)) {
    return (AsyncResult){.status = ASYNC_ERROR, .error = stateResult.error};
  }
  ...
  if (state->overlapped.hEvent == NULL) {
    ...
    return (AsyncResult){.status = ASYNC_ERROR,
                         .error = fun_error_result(
                             GetLastError(), "Failed to create event handle")};
  }
  ...
  if (state->dir_handle == INVALID_HANDLE_VALUE) {
    ...
    return (AsyncResult){.status = ASYNC_ERROR,
                         .error = fun_error_result(
                             error, "Failed to open directory for monitoring")};
  }
  ...
  if (monitoring_thread == NULL) {
    ...
    return (AsyncResult){
        .status = ASYNC_ERROR,
        .error = fun_error_result(error, "Failed to create monitoring thread")};
  }
...
AsyncResult fun_unregister_file_change_notification(String filePath) {
  ...
  // This API call would actually remove a specific registration if we had a
  // registry of file registrations
