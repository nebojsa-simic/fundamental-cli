									   ...
									   size_t *read_pos, size_t *count)
{
	...
	size_t bytes_to_read = bytes_available < buffer_size ? bytes_available :
														   buffer_size;
...
AsyncStatus platform_process_poll(AsyncResult *result)
{
	...
	if (ret > 0) {
		...
		if (stdout_fd >= 0) {
			read_pipe_to_buffer(stdout_fd, result->process.stdout_buffer,
		...
		if (stderr_fd >= 0) {
			read_pipe_to_buffer(stderr_fd, result->process.stderr_buffer,
