...
AsyncStatus platform_process_poll(AsyncResult *result) {
  ...
  if (exit_code != STILL_ACTIVE) {
    ...
    if (stdout_pipe != NULL) {
      read_pipe_to_buffer(
          stdout_pipe, result->process.stdout_buffer,
          PROCESS_STDOUT_BUFFER_SIZE, &result->process.stdout_write_pos,
          &result->process.stdout_read_pos, &result->process.stdout_count);
    ...
    if (stderr_pipe != NULL) {
      read_pipe_to_buffer(
          stderr_pipe, result->process.stderr_buffer,
          PROCESS_STDERR_BUFFER_SIZE, &result->process.stderr_write_pos,
          &result->process.stderr_read_pos, &result->process.stderr_count);
  ...
  if (stdout_pipe != NULL) {
    read_pipe_to_buffer(
        stdout_pipe, result->process.stdout_buffer, PROCESS_STDOUT_BUFFER_SIZE,
  ...
  if (stderr_pipe != NULL) {
    read_pipe_to_buffer(
        stderr_pipe, result->process.stderr_buffer, PROCESS_STDERR_BUFFER_SIZE,
